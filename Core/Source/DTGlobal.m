...
//  
//
//  Created by TanHuynh on 2020/11/20.
//
...
+ (BOOL)isIgnorableWhitespace:(NSString *)string
{
    ...
    NSString *tmpStr = [string stringByTrimmingCharactersInSet:whitespaceCharacterSet];
...
{
    float result = 1;
    sscanf([string UTF8String], "%f", &result);
...
+ (NSString *)stringByNormalizingWhitespace:(NSString *)string
{
    ...
    [string getCharacters:_characters range:NSMakeRange(0, stringLength)];
    ...
    for (NSInteger i = 0; i<stringLength; i++)
    {
        ...
        unichar oneChar = _characters[i]; // [string characterAtIndex:i];
...
+ (NSString *)stringByAddingHTMLEntities:(NSString *)string
{
    ...
    for (NSUInteger i = 0; i<[string length]; i++)
    {
        ...
        else
        {
            ...
            else if (CFStringIsSurrogateHighCharacter(oneChar) && i < [string length]-1)
            {
                i++;
...
#else
    return [string stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet ASCIICharacterSet]];
#endif
}
